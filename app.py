...
MAIL_FROM = os.environ.get('MAIL_FROM', 'deskflow@resend.dev')
...
def dashboard():
    ...
    if f_assignee:
        query = query.filter(Ticket.assigned_to == int(f_assignee))
    ...
    return render_template('dashboard.html',
        ...
        closed_tickets    =[t for t in all_tickets if t.status == 'closed' and t.updated_at >= cutoff],
        stats=stats, categories=categories, teams=teams, users=users,
        q=q, f_status=f_status, f_priority=f_priority,
        f_category=f_category, f_team=f_team, f_assignee=f_assignee
    )
...
def test_email():
    if not current_user.is_admin:
        return 'Admin only'
    key = os.environ.get('RESEND_API_KEY', 'NOT SET')
    resp = requests.post(
        'https://api.resend.com/emails',
        headers={'Authorization': f'Bearer {key}', 'Content-Type': 'application/json'},
        json={'from': 'deskflow@resend.dev', 'to': [current_user.email], 'subject': 'DeskFlow test email', 'html': '<p>Email is working!</p>'}
    )
